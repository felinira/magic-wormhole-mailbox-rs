...
pub enum ClientMessage {
    ...
    #[display(fmt = "Add {{ phase: {}, body: {:?} }}", phase, body)]
    Add {
        phase: Phase,
        #[serde(
            serialize_with = "hex::serde::serialize",
...
mod test {
    ...
    #[test]
    fn test_claim() {
        let m1 = ClientMessage::claim("nameplate1");
        let s = serde_json::to_string(&m1).unwrap();
        let m2: Value = from_str(&s).unwrap();
    ...
    #[test]
    fn test_release() {
        let m1 = ClientMessage::release("nameplate1");
        let s = serde_json::to_string(&m1).unwrap();
        let m2: Value = from_str(&s).unwrap();
    ...
    #[test]
    fn test_open() {
        let m1 = ClientMessage::open(Mailbox(String::from("mailbox1")));
        let s = serde_json::to_string(&m1).unwrap();
        let m2: Value = from_str(&s).unwrap();
    ...
    #[test]
    fn test_add() {
        let m1 = ClientMessage::add(Phase("phase1".into()), b"body".to_vec());
        let s = serde_json::to_string(&m1).unwrap();
        let m2: Value = from_str(&s).unwrap();
        assert_eq!(
            m2,
    ...
    #[test]
    fn test_close() {
        let m1 = ClientMessage::close(Mailbox(String::from("mailbox1")), Mood::Happy);
        let s = serde_json::to_string(&m1).unwrap();
        let m2: Value = from_str(&s).unwrap();
        assert_eq!(
            m2,
    ...
    fn test_close_errory() {
        let m1 = ClientMessage::close(Mailbox(String::from("mailbox1")), Mood::Errory);
        let s = serde_json::to_string(&m1).unwrap();
        let m2: Value = from_str(&s).unwrap();
        assert_eq!(
            m2,
    ...
    fn test_close_scared() {
        let m1 = ClientMessage::close(Mailbox(String::from("mailbox1")), Mood::Scared);
        let s = serde_json::to_string(&m1).unwrap();
        let m2: Value = from_str(&s).unwrap();
        assert_eq!(
            m2,
    ...
    fn test_submit_permissions() {
        let m = ClientMessage::SubmitPermission(SubmitPermission::Hashcash {
            stamp: "stamp".into(),
        });
    ...
    fn test_message() {
        ...
        let m = serde_json::from_str(s).unwrap();
        match m {
            ServerMessage::Message(EncryptedMessage {
                side: _s,
                phase: _p,
                body: _b,
                //id: i
